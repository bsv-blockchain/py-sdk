...
class TestVerifyScripts:
    ...
    async def test_verify_scripts_with_missing_source_transaction(self):
        ...
        tx = Transaction(
            ...
            [TransactionOutput(locking_script=P2PKH().lock(priv_key.address()), satoshis=1000)],
        )

        # verify_scripts should raise ValueError when source transaction is missing
        with pytest.raises(
            ValueError,
            match="Verification failed because the input at index 0 of transaction .* is missing an associated source transaction",
        ):
