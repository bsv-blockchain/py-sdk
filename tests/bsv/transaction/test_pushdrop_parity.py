...
def test_pushdrop_lock_includes_signature_by_default():
    ...
    script = pd.lock(None, fields, proto, "kid", {"type": CounterpartyType.ANYONE})
...
def test_pushdrop_decode_restores_small_ints():
    ...
    proto = {"securityLevel": 2, "protocol": "pushdrop"}
    pub = wallet.get_public_key(None, {"protocolID": proto, "keyID": "k", "counterparty": {"type": CounterpartyType.SELF}, "forSelf": True}, "org")
...
def test_pushdrop_lock_after_and_decode():
    ...
    fields = [b"x", b"y", b"z"]
    proto = {"securityLevel": 2, "protocol": "pushdrop"}
    script = pd.lock(None, fields, proto, "kid", {"type": CounterpartyType.ANYONE}, lock_position="after")
...
def test_pushdrop_include_signature_flag_changes_field_count():
    ...
    fields = [b"d1", b"d2"]
    proto = {"securityLevel": 2, "protocol": "pushdrop"}
    s_with = pd.lock(None, fields, proto, "kid", {"type": CounterpartyType.ANYONE}, include_signature=True)
    s_without = pd.lock(None, fields, proto, "kid", {"type": CounterpartyType.ANYONE}, include_signature=False)
...
def test_pushdrop_unlock_sign_and_estimate():
    ...
    fields = [b"val"]
    script = pd.lock(None, fields, proto, "kid", {"type": CounterpartyType.ANYONE})
    unlock = pd.unlock(proto, "kid", {"type": CounterpartyType.ANYONE}, sign_outputs='all', prev_txid="00" * 32, prev_vout=0, prev_satoshis=1, prev_locking_script=script)
...
def test_pushdrop_sighash_modes_match_range():
    ...
    for mode in ("all", "none", "single"):
        unlock = pd.unlock(proto, "kid", {"type": CounterpartyType.ANYONE}, sign_outputs=mode, prev_txid="00" * 32, prev_vout=0, prev_satoshis=1, prev_locking_script=script)
        sigpush = unlock.sign(None, b"dummy_tx_bytes", 0)
...
def test_pushdrop_sighash_flag_values_and_anyonecanpay():
    ...
    for mode, acp, expected_flag in cases:
        unlock = pd.unlock(proto, "kid", {"type": CounterpartyType.ANYONE}, sign_outputs=mode, anyone_can_pay=acp, prev_txid="00" * 32, prev_vout=0, prev_satoshis=1, prev_locking_script=script)
        sigpush = unlock.sign(None, b"dummy_tx_bytes", 0)
...
def test_sign_action_sighash_bip143_acp_parity():
    ...
    script_before = pd.lock(None, fields, proto, "kid", {"type": CounterpartyType.ANYONE}, lock_position="before")
    script_after = pd.lock(None, fields, proto, "kid", {"type": CounterpartyType.ANYONE}, lock_position="after")
    ...
    for lock_position, sighash_mode, acp, expected_flag in cases:
        ...
        unlocker = make_pushdrop_unlocker(
            wallet, proto, "kid", {"type": CounterpartyType.ANYONE},
