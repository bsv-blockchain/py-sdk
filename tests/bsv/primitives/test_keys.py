...
def test_verify():
    ...
    der: bytes = bytes.fromhex(
        "304402207e2c6eb8c4b20e251a71c580373a2836e209c50726e5f8b0f4f59f8af00eee1a"
    ...
    der: bytes = bytes.fromhex(
        "3043022053b1f5a28a011c60614401eeef88e49c676a098ce36d95ded1b42667f40efa37"
        "021f4de6703f8c74b0ce5dad617c00d1fb99580beb7972bf681e7215911c3648de"
    )
    assert private_key.verify(der, digest2)
    der: bytes = bytes.fromhex(
        "3045022100b9f293781ae1e269591df779dbadb41b9971d325d7b8f83d883fb55f2cb3ff76"
...
def test_ecdh():
    ...
    assert alice_shared == bob_shared, "Shared secrets should match (ECDH property)"
    ...
    assert secret_from_pub == secret_from_priv, "Public key and private key ECDH methods should yield same result"
    ...
    assert alice_charlie_shared != alice_shared, "Different key pairs should produce different shared secrets"
...
def test_encryption():
    ...
    encrypted = (
        "QklFMQPkjNG3xxnfRv7oUDjUYPH2VN3VFrcglCcwmeYpJpsjRKnfl/XsS+dOg"
    ...
    assert decrypted_new == plain, f"Encrypt/decrypt roundtrip should preserve plaintext, got '{decrypted_new}'"
