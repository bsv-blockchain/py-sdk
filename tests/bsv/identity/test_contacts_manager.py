...
class TestContactsManager:
    ...
    def setup_method(self):
        ...
        self.wallet.list_outputs = Mock(return_value={"outputs": [], "BEEF": None})
        self.wallet.create_hmac = Mock(return_value={"hmac": b"\x01\x02\x03\x04"})
        self.wallet.encrypt = Mock(return_value={"ciphertext": b"encrypted_contact_data"})
        self.wallet.decrypt = Mock(return_value={"plaintext": b'{"identityKey":"test-key","name":"Test Contact"}'})
        self.wallet.get_public_key = Mock(
            return_value={"publicKey": "02a1633cafb311f41c1137864d7dd7cf2d5c9e5c2e5b5f5a5d5c5b5a59584f5e5fac"}
        )
        self.wallet.create_signature = Mock(return_value={"signature": b"dummy_signature_for_testing_purposes_32bytes"})
        self.wallet.create_action = Mock(return_value={"tx": b"transaction_bytes"})
    ...
    def test_should_get_contacts_by_identity_key(self):
        ...
        self.wallet.list_outputs.return_value = {
            "outputs": [
                {"outpoint": "txid1.0", "lockingScript": "mock_script", "customInstructions": '{"keyID":"test-key-id"}'}
            ],
            "BEEF": b"mock_beef",
        }
    ...
    def test_should_save_new_contact(self):
        ...
        contact = {"identityKey": "new-contact-key", "name": "New Contact", "avatarURL": "avatar.png"}
    ...
    def test_should_update_existing_contact(self):
        ...
        self.wallet.list_outputs.return_value = {"outputs": [existing_output], "BEEF": b"mock_beef"}

        contact = {"identityKey": "existing-contact-key", "name": "Updated Contact", "avatarURL": "new_avatar.png"}
    ...
    def test_should_delete_contact(self):
        ...
        self.contacts_manager.get_contacts = Mock(
            return_value=[{"identityKey": identity_key, "name": "Contact to Delete"}]
        )
        self.wallet.list_outputs.return_value = {
            "outputs": [{"outpoint": "txid1.0", "lockingScript": "mock_script"}],
            "BEEF": b"mock_beef",
        }
