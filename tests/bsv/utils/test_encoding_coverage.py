...
def test_hex_encode_empty():
    ...
    try:
        from bsv.utils.binary import to_hex
        result = to_hex(b'')
        assert result == ""
    except ImportError:
...
def test_base64_encode_empty():
    ...
    try:
        from bsv.utils.binary import to_base64
        result = to_base64([])
        assert result == ""
    except ImportError:
        pytest.skip("Binary functions not available")
...
def test_base64_encode_value():
    ...
    try:
        from bsv.utils.binary import to_base64
        result = to_base64([116, 101, 115, 116])  # 'test' as list of ints
        assert len(result) > 0
    except ImportError:
        pytest.skip("Binary functions not available")
...
def test_base64_decode_empty():
    ...
    try:
        from bsv.utils.binary import to_bytes
        result = to_bytes("", 'base64')
        assert result == b''
    except ImportError:
        pytest.skip("Binary functions not available")
