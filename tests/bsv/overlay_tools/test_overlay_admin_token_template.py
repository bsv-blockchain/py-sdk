...
class TestDecode:
    ...
    def test_decode_ship_advertisement(self):
        ...
        with patch("bsv.overlay_tools.overlay_admin_token_template.PushDrop.decode") as mock_decode:
            mock_decode.return_value = {"fields": [b"SHIP", b"\x01\x02\x03", b"example.com", b"topic1"]}
    ...
    def test_decode_slap_advertisement(self):
        ...
        with patch("bsv.overlay_tools.overlay_admin_token_template.PushDrop.decode") as mock_decode:
            mock_decode.return_value = {"fields": [b"SLAP", b"\xab\xcd\xef", b"service.example.com", b"service1"]}
    ...
    def test_decode_with_string_fields(self):
        ...
        with patch("bsv.overlay_tools.overlay_admin_token_template.PushDrop.decode") as mock_decode:
            mock_decode.return_value = {"fields": ["SHIP", "0123456789abcdef", "test.com", "topic"]}
    ...
    def test_decode_invalid_protocol(self):
        ...
        with patch("bsv.overlay_tools.overlay_admin_token_template.PushDrop.decode") as mock_decode:
            mock_decode.return_value = {"fields": [b"INVALID", b"\x01", b"test.com", b"topic"]}
    ...
    def test_decode_insufficient_fields(self):
        ...
        with patch("bsv.overlay_tools.overlay_admin_token_template.PushDrop.decode") as mock_decode:
            mock_decode.return_value = {"fields": [b"SHIP", b"\x01", b"test.com"]}  # Only 3 fields
...
class TestLock:
    ...
    async def test_lock_ship_advertisement(self):
        ...
        with (
            patch("bsv.overlay_tools.overlay_admin_token_template.PushDrop") as MockPushDrop,
...
class TestEdgeCases:
    ...
    def test_decode_with_unicode_domain(self):
        ...
        with patch("bsv.overlay_tools.overlay_admin_token_template.PushDrop.decode") as mock_decode:
            mock_decode.return_value = {"fields": [b"SHIP", b"\x01", "中文.com".encode(), b"topic"]}
    ...
    def test_decode_with_long_identity_key(self):
        ...
        with patch("bsv.overlay_tools.overlay_admin_token_template.PushDrop.decode") as mock_decode:
            long_key = b"\xff" * 64
            mock_decode.return_value = {"fields": [b"SLAP", long_key, b"test.com", b"service"]}
