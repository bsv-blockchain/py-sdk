...
from bsv.hd.bip44 import derive_xkeys_from_xkey, derive_xprvs_from_mnemonic
...
master_xprv = (
    "xprv9s21ZrQH143K4SSfHuCgyJKsown12SFNpzCf3XYJT67mkaVaWCCBqiGBRZRmgk2ypzXoWzAccyVPGBW69A6LLRMnbY6GZ27q6UkiJDnPjhT"
)
master_xpub = (
    "xpub661MyMwAqRbcGvX8PvjhLSGcMycVRtyECD8Fquwv1RekdNpj3jWSPWafGsdNa6TNVmDN9HpPe2tRPofzHTYAUeQFUsAQpzuVSDDyUCt975T"
)

# m/0
normal_xprv = (
    "xprv9v35D6cvdU6R1d3UuY6bbR87h6pJLQn3kXY9jwGXhqTX129XT5jZnEyTDoDKnoE9k7HSK7MNv7E3gEGkt4Bp7BkcgHgXUHzQHXueD1t2vRj"
)
normal_xpub = (
    "xpub692Rcc9pTqeiE77x1ZdbxZ4rF8enjsVu7kTkYKg9GAzVspUfzd3pL3Hw56Fkgg4vrhayKd6k33uiJgmicfiKf2T1E5brXQLeQni1ake7uSv"
)

# m/0'
hardened_xprv = (
    "xprv9v35D6d4y8dP9r1N2koQ49hwzk8EDT4msMFAXGertWPxQDByPqZ1e3k6U34kwU4iCnur3UcxX4SvaDFcrubYd3ktsfpCraGmWpqDq4fm1SJ"
)
hardened_xpub = (
    "xpub692Rcc9xoWBgNL5q8nLQRHegYmxicundEaAmKf4USqvwH1X7wNsGBr4aKHLeKDA5ghqECjBErUwLaYZ6As5PpqsFJbZD3jyBWrk6QKG8QQX"
)
...
def test_mnemonic():
    ...
    mnemonic = (
        "beauty setup nation bright drop fat duty divorce same early grid mandate "
    ...
    assert (
        ckd(master_xprv_from_seed(seed_from_mnemonic(mnemonic, "zh-cn")), path).address()
        == "1C5XJhzRNDDuPNzETmJFFhkU46s1bBFqyV"
    )
    ...
    assert (
        ckd(master_xprv_from_seed(seed_from_mnemonic(mnemonic, "zh-cn")), path).address()
        == "1GeiN188BR499mp4JvT1EHD7MVUZ1jJVMj"
    )
    ...
    assert (
        ckd(master_xprv_from_seed(seed_from_mnemonic(mnemonic, "zh-cn")), path).address()
        == "1PUaGha3pSPUwCT7JTLTXUdnL9wbvibU1u"
    )
...
def test_derive():
    ...
    assert [
        xprv.private_key().wif() for xprv in derive_xprvs_from_mnemonic(mnemonic, "1'", "3'", path="m/44'/0'/0'")
    ] == [
    ...
    assert [
        xprv.private_key().wif() for xprv in derive_xprvs_from_mnemonic(mnemonic, 0, 2, change=1, path="m/44'/0'/0'")
    ] == [
    ...
    assert [
        xprv.private_key().wif() for xprv in derive_xprvs_from_mnemonic(mnemonic, 0, 2, change="0'", path="m/44'/0'/0'")
    ] == [
    ...
    assert [
        xprv.private_key().wif()
        for xprv in derive_xprvs_from_mnemonic(mnemonic, 0, 2, passphrase=TEST_PASSPHRASE_BITCOIN, path="m/44'/0'/0'")
    ] == [
    ...
    assert [
        xprv.private_key().wif()
        for xprv in derive_xprvs_from_mnemonic(mnemonic, 0, 2, lang="zh-cn", path="m/44'/0'/0'")
    ] == [
