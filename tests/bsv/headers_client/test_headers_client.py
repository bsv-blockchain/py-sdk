...
class TestHeadersClientGetMerkleRoots:
    ...
    async def test_get_merkle_roots_success(self):
        ...
        mock_client.set_response(
            ...
            HttpResponse(
                ok=True,
                status_code=200,
                json_data={"data": {"content": expected_roots, "page": {"lastEvaluatedKey": ""}}},
            ),
        )
    ...
    async def test_get_merkle_roots_with_last_evaluated_key(self):
        ...
        mock_client.set_response(
            "/api/v1/chain/merkleroot",
            HttpResponse(
                ok=True, status_code=200, json_data={"data": {"content": [], "page": {"lastEvaluatedKey": ""}}}
            ),
        )
    ...
    async def test_get_merkle_roots_error(self):
        ...
        mock_client = MockHttpClient()
        mock_client.set_response("/api/v1/chain/merkleroot", HttpResponse(ok=False, status_code=500, json_data={}))
    ...
    async def test_get_merkle_roots_invalid_json(self):
        ...
        mock_client = MockHttpClient()
        mock_client.set_response(
            "/api/v1/chain/merkleroot", HttpResponse(ok=True, status_code=200, json_data={"invalid": "json"})
        )
...
class TestHeadersClientWebhooks:
    ...
    async def test_register_webhook_success(self):
        ...
        mock_client = MockHttpClient()
        mock_client.set_response(
            "/api/v1/webhook", HttpResponse(ok=True, status_code=200, json_data={"data": expected_webhook})
        )
    ...
    async def test_register_webhook_error(self):
        ...
        mock_client = MockHttpClient()
        mock_client.set_response(
            "/api/v1/webhook", HttpResponse(ok=False, status_code=400, json_data={"error": "Invalid webhook URL"})
        )

        client = HeadersClient("https://test.com", "test-api-key", mock_client)

        with pytest.raises(
            Exception, match="failed to register webhook: status=400, body={'error': 'Invalid webhook URL'}"
        ):
    ...
    async def test_unregister_webhook_success(self):
        ...
        mock_client = MockHttpClient()
        mock_client.set_response("/api/v1/webhook", HttpResponse(ok=True, status_code=200, json_data={}))
    ...
    async def test_unregister_webhook_error(self):
        ...
        mock_client = MockHttpClient()
        mock_client.set_response(
            "/api/v1/webhook", HttpResponse(ok=False, status_code=404, json_data={"error": "Webhook not found"})
        )

        client = HeadersClient("https://test.com", "test-api-key", mock_client)

        with pytest.raises(
            Exception, match="failed to unregister webhook: status=404, body={'error': 'Webhook not found'}"
        ):
...
class TestHeadersClientChainTracker:
    ...
    async def test_is_valid_root_for_height(self):
        ...
        mock_client.set_response(
            ...
            HttpResponse(ok=True, status_code=200, json_data={"data": {"confirmationState": "CONFIRMED"}}),
        )
    ...
    async def test_current_height(self):
        ...
        mock_client.set_response(
            ...
            HttpResponse(
                ok=True, status_code=200, json_data={"data": {"height": 850000, "state": "LONGEST_CHAIN", "header": {}}}
            ),
        )
