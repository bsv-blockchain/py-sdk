...
from ..constants import (
    ...
    BIP32_SEED_BYTE_LENGTH,
    NETWORK_XPRV_PREFIX_DICT,
    NETWORK_XPUB_PREFIX_DICT,
    PUBLIC_KEY_COMPRESSED_PREFIX_LIST,
    XKEY_BYTE_LENGTH,
    XKEY_PREFIX_LIST,
    XPRV_PREFIX_NETWORK_DICT,
    XPUB_PREFIX_NETWORK_DICT,
    Network,
)
...
class Xkey:
    ...
    def __init__(self, xkey: str | bytes):
...
class Xpub(Xkey):
    def __init__(self, xpub: str | bytes):
        super().__init__(xpub)
...
class Xprv(Xkey):
    def __init__(self, xprv: str | bytes):
        super().__init__(xprv)
    ...
    def ckd(self, index: int | str | bytes) -> "Xprv":
        if isinstance(index, int):
            index = index.to_bytes(4, "big")
        elif isinstance(index, str):
            index = bytes.fromhex(index)
    ...
    @classmethod
    def from_seed(cls, seed: str | bytes, network: Network = Network.MAINNET):
        """
...
def step_to_index(step: str | int) -> int:
    """
...
def master_xprv_from_seed(seed: str | bytes, network: Network = Network.MAINNET) -> Xprv:
...
def bip32_derive_xprv_from_mnemonic(
    mnemonic: str,
    lang: str = "en",
    passphrase: str = "",
    prefix: str = "mnemonic",
...
def bip32_derive_xprvs_from_mnemonic(
    mnemonic: str,
    index_start: str | int,
    index_end: str | int,
    lang: str = "en",
